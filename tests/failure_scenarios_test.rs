...
async fn test_postgres_connection_failure() {
    ...
    client
        .execute(
            "INSERT INTO failure_test (status) VALUES ('before_failure')",
            &[],
        )
        .await
        .unwrap();
    ...
    client
        .execute(
            "INSERT INTO failure_test (status) VALUES ('after_recovery')",
            &[],
        )
        .await
        .unwrap();
...
async fn test_kafka_connection_failure() {
    ...
    client
        .execute(
            "INSERT INTO failure_test (status) VALUES ('before_kafka_failure')",
            &[],
        )
        .await
        .unwrap();
    ...
    for i in 0..5 {
        client
            .execute(
                &format!(
                    "INSERT INTO failure_test (status) VALUES ('during_failure_{}')",
                    i
                ),
                &[],
            )
    ...
    let _proxy_handle =
        start_kafka_proxy(proxy_port, "localhost:9092", proxy_running.clone()).await;
    info!("Kafka proxy restarted - connection restored");

    // Insert data after recovery
    client
        .execute(
            "INSERT INTO failure_test (status) VALUES ('after_kafka_recovery')",
            &[],
        )
        .await
        .unwrap();
        ...
        if let Ok(Ok(message)) = timeout(Duration::from_secs(1), consumer.recv()).await {
            if let Some(payload) = message.payload() {
                ...
                if json["op"] == "INSERT" && json["table"] == "failure_test" {
                    ...
                    info!(
                        "Received message {}/{}: {:?}",
                        received_count, expected_count, json["after"]["status"]
                    );
                }
            }
        }
...
async fn test_replicator_crash_recovery() {
    ...
    let checkpoint_file = format!(
        "/tmp/pg_capture_crash_test_{}.checkpoint",
        std::process::id()
    );
...
async fn test_malformed_replication_data() {
    ...
    client
        .execute("DROP TABLE IF EXISTS complex_types", &[])
        .await
        .ok();
    client
        .execute(
    ...
    client
        .execute(
            &format!(
                "ALTER PUBLICATION {} ADD TABLE complex_types",
                test_config.postgres.publication
            ),
            &[],
        )
        .await
    ...
    let test_cases: Vec<String> = vec![
        ...
        // Large text
        format!(
            "INSERT INTO complex_types (large_text) VALUES ('{}')",
            "x".repeat(10000)
        ),
        // NULL values
    ...
    client
        .execute(
            "UPDATE complex_types SET json_data = '{\"updated\": true}'::jsonb WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
    client
        .execute("DELETE FROM complex_types WHERE id = 2", &[])
        .await
        .unwrap();
    ...
    assert!(
        !replicator_handle.is_finished(),
        "Replicator crashed during edge case testing"
    );
    ...
    while start.elapsed() < timeout_duration {
        if let Ok(Ok(message)) = timeout(Duration::from_millis(500), consumer.recv()).await {
            if let Some(payload) = message.payload() {
                ...
                let json: Value =
                    serde_json::from_slice(payload).expect("Replicator produced invalid JSON");
    ...
    assert!(
        message_count >= 9,
        "Expected at least 9 messages, got {}",
        message_count
    );
    ...
    replicator_handle.abort();
    client
        .execute("DROP TABLE complex_types CASCADE", &[])
        .await
        .ok();
    cleanup_failure_test(&client).await;
}
...
async fn setup_failure_test() -> (Client, Config) {
    ...
    client.execute(&format!("SELECT pg_drop_replication_slot('{}') WHERE EXISTS (SELECT 1 FROM pg_replication_slots WHERE slot_name = '{}')", 
        ...
        .await.ok();
    client
        .execute("DROP TABLE IF EXISTS failure_test", &[])
        .await
        .ok();
    ...
    client
        .execute(
            &format!(
                "CREATE PUBLICATION {} FOR TABLE failure_test",
...
async fn cleanup_failure_test(client: &Client) {
    ...
    client
        .execute("DROP TABLE IF EXISTS failure_test CASCADE", &[])
        .await
        .ok();
    client
        .execute("DROP PUBLICATION IF EXISTS failure_publication", &[])
        .await
        .ok();
    client.execute(&format!("SELECT pg_drop_replication_slot('{}') WHERE EXISTS (SELECT 1 FROM pg_replication_slots WHERE slot_name = '{}')", 
                           slot_name, slot_name), &[])
...
async fn create_failure_consumer(kafka_config: &KafkaConfig) -> StreamConsumer {
    let consumer: StreamConsumer = ClientConfig::new()
        ...
        .set(
            "group.id",
            format!("failure_consumer_{}", std::process::id()),
        )
