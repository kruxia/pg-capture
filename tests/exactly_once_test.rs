...
async fn test_no_duplicate_messages() {
    ...
    assert_eq!(message_ids.len(), num_messages, 
               "Expected {} unique messages, got {}", num_messages, message_ids.len());
    assert_eq!(duplicate_count, 0, 
               "Found {} duplicate messages", duplicate_count);

    // Verify no messages were lost
    for i in 0..num_messages {
        let msg_id = format!("msg_{}", i);
        assert!(message_ids.contains_key(&msg_id), 
                "Missing message: {}", msg_id);
        assert_eq!(*message_ids.get(&msg_id).unwrap(), 1,
                "Message {} was delivered {} times", msg_id, message_ids.get(&msg_id).unwrap());
    }
...
async fn test_checkpoint_consistency() {
    ...
    // Verify counts
    assert_eq!(batch1_count, 50, "Expected 50 batch1 messages, got {}", batch1_count);
    assert_eq!(batch2_count, 50, "Expected 50 batch2 messages, got {}", batch2_count);
...
async fn test_transaction_boundaries() {
    ...
    client.execute("INSERT INTO exactly_once_test (unique_id, data) VALUES ('tx1_1', 'first')", &[])
        .await.unwrap();
    client.execute("INSERT INTO exactly_once_test (unique_id, data) VALUES ('tx1_2', 'second')", &[])
        .await.unwrap();
    client.execute("INSERT INTO exactly_once_test (unique_id, data) VALUES ('tx1_3', 'third')", &[])
        .await.unwrap();
    client.execute("COMMIT", &[]).await.unwrap();

    // Test 2: Rolled back transaction (should not appear in Kafka)
    client.execute("BEGIN", &[]).await.unwrap();
    client.execute("INSERT INTO exactly_once_test (unique_id, data) VALUES ('tx2_1', 'rolled_back')", &[])
        .await.unwrap();
    client.execute("ROLLBACK", &[]).await.unwrap();

    // Test 3: Another committed transaction
    client.execute("BEGIN", &[]).await.unwrap();
    client.execute("INSERT INTO exactly_once_test (unique_id, data) VALUES ('tx3_1', 'committed')", &[])
        .await.unwrap();
    client.execute("COMMIT", &[]).await.unwrap();
    ...
    assert!(!received_ids.contains(&"tx2_1".to_string()), 
...
async fn setup_exactly_once_test() -> (Client, Config) {
    ...
    client.execute(&format!("SELECT pg_drop_replication_slot('{}') WHERE EXISTS (SELECT 1 FROM pg_replication_slots WHERE slot_name = '{}')", 
        ...
        .await.ok();
    client.execute("DROP TABLE IF EXISTS exactly_once_test", &[])
        .await.ok();
    ...
    // Create publication
    client.execute(
        &format!("CREATE PUBLICATION {} FOR TABLE exactly_once_test", db_config.publication),
        &[]
    ).await.unwrap();
...
async fn cleanup_exactly_once_test(client: &Client) {
    ...
    client.execute("DROP TABLE IF EXISTS exactly_once_test CASCADE", &[]).await.ok();
    client.execute("DROP PUBLICATION IF EXISTS exactly_once_publication", &[]).await.ok();
    client.execute(&format!("SELECT pg_drop_replication_slot('{}') WHERE EXISTS (SELECT 1 FROM pg_replication_slots WHERE slot_name = '{}')", 
                           slot_name, slot_name), &[])
        .await.ok();
}
...
async fn create_exactly_once_consumer(kafka_config: &KafkaConfig) -> StreamConsumer {
    let consumer: StreamConsumer = ClientConfig::new()
        ...
        .set("group.id", format!("exactly_once_consumer_{}", std::process::id()))
