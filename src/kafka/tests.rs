...
mod tests {
    ...
    fn test_key_strategy_extraction() {
        ...
        assert_eq!(strategy.extract_key(&event), Some("user@example.com".to_string()));
    ...
    async fn test_send_change_event() {
        ...
        let result = producer.send_change_event(&event, &key_strategy, &serializer).await;
        assert!(result.is_ok());
    }
    ...
    async fn test_batch_sending() {
        ...
        let results = producer.send_change_events_batch(&events, &key_strategy, &serializer).await.unwrap();
    ...
    fn test_nested_field_extraction() {
        ...
        assert_eq!(strategy.extract_key(&event), Some("nested@example.com".to_string()));
    }
